...
class MQTTAsyncSource(Sourcer):
    ...
    async def start_mqtt_consumer(self):
        ...
        logger.info("Starting MQTT consumer for broker=%s, port=%s, topic=%s",
            ...
            while True:
                try:
                    async with Client(self.broker, self.port) as client:
                        ...
                        logger.info("Successfully subscribed to MQTT topic: %s", self.topic)
                    ...
                    logger.error("MQTT consumer error: %s. Retrying in 5 seconds...", e)
    ...
    async def read_handler(self, datum: ReadRequest, output: NonBlockingIterator):
        ...
        for _ in range(datum.num_records):
            ...
            try:
                ...
                logger.info("Sending MQTT message: %s", payload)
...
if __name__ == "__main__":
    ...
    logger.info("Configuring MQTT Source: broker=%s, port=%s, topic=%s", broker, port, topic)
